...
describe('Exam Service', () => {
  ...
  describe('createExam', () => {
    ...
    test('creates exam with client', async () => {
      const mockClient = {};
      await ExamService.createExam({}, 't1', 'c1', mockClient);
      expect(ExamModel.createExam).toHaveBeenCalledWith(expect.any(Object), mockClient);
    });
  });
  ...
  describe('updateExam', () => {
    test('updates exam successfully', async () => {
      ...
      const result = await ExamService.updateExam(examUuid, updateData);

      expect(result).toEqual(updated);
      expect(ExamModel.updateExam).toHaveBeenCalledWith(examUuid, updateData);
    });
  });
